...
import { collection, query, where, getDocs, deleteDoc, doc, orderBy, limit, startAfter, DocumentSnapshot, QueryDocumentSnapshot } from "firebase/firestore";
...
export default function SermonsPage() {
  ...
  const loadSermons = async (loadMore = false) => {
    ...
    // Se estiver carregando mais, usar loadingMore, senão usar loading
    if (loadMore) {
      setLoadingMore(true);
    } else {
      setLoading(true);
    ...
    try {
      ...
      let sermonsQuery = query(
        ...
        orderBy("date", "desc"), // Ordenar no Firestore para paginação funcionar
        limit(PAGE_SIZE)
      );

      // Se estiver carregando mais, adicionar startAfter
      if (loadMore && lastDoc) {
      ...
      const newSermons: Sermon[] = [];
      snapshot.forEach((doc) => {
        newSermons.push({ id: doc.id, ...doc.data() } as Sermon);
      });

      // Atualizar lista: adicionar se loadMore, substituir se não
      if (loadMore) {
        const updatedSermons = [...sermons, ...newSermons];
        setSermons(updatedSermons);
        // Aplicar filtros imediatamente para atualizar filteredSermons
        applyFilters(updatedSermons);
      } else {
        setSermons(newSermons);
        // Aplicar filtros imediatamente para atualizar filteredSermons
        applyFilters(newSermons);
      }
  ...
  const applyFilters = (sermonsList: Sermon[]) => {
    let filtered = [...sermonsList];
    ...
    // Sempre atualizar filteredSermons, mesmo quando dateFilter é "all"
    setFilteredSermons(filtered);
  };

  // Filtrar sermões quando filtros ou sermões mudarem
  useEffect(() => {
    applyFilters(sermons);
  ...
  const handleDeleteSermon = async (sermonId: string) => {
    ...
    try {
      ...
      // Recarregar do início após deletar
      await loadSermons(false);
    } catch (error) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {hasMore && sermons.length > 0 && !loading && (
          <div className="flex justify-center mt-8">
            <button
              onClick={() => loadSermons(true)}
              disabled={loadingMore}
              className="btn-violet flex items-center gap-2 hover-lift disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loadingMore ? (
                <>
