...
import { collection, getDocs, addDoc, query, orderBy, doc, getDoc, updateDoc, arrayUnion, arrayRemove, where, limit, startAfter, QueryDocumentSnapshot } from "firebase/firestore";
...
export default function ReflectionsPage() {
  ...
  const loadReflections = async (loadMore = false) => {
    // Se ordenar por likes, carregar mais dados (até 100) e ordenar localmente
    // Se ordenar por data, usar paginação do Firestore
    const maxItemsForLikes = 100;
    ...
    try {
      let reflectionsQuery;
      
      if (sortBy === "date") {
        // Paginação para ordenação por data
        let baseQuery = query(
        ...
        reflectionsQuery = baseQuery;
      } else {
        // Para ordenação por likes, carregar mais itens de uma vez
        let baseQuery = query(
          collection(db, "reflections"),
          orderBy("createdAt", "desc"),
          limit(loadMore ? PAGE_SIZE : maxItemsForLikes)
        );
      ...
      // Ordenar por likes se necessário (apenas se sortBy === "likes")
      if (sortBy === "likes") {
        newReflections.sort((a, b) => {
          if (b.likes !== a.likes) {
      ...
      // Carregar contagens de comentários para todas as reflexões carregadas
      const countPromises = newReflections.map(reflection => 
        loadCommentCount(reflection.id)
      );
      ...
      if (sortBy === "date") {
        ...
        // Para ordenação por likes, limitar a 100 itens
        const lastDocument = snapshot.docs[snapshot.docs.length - 1];
        setLastDoc(lastDocument);
        const totalAfterLoad = loadMore ? reflections.length + newReflections.length : newReflections.length;
        setHasMore(snapshot.docs.length === PAGE_SIZE && totalAfterLoad < maxItemsForLikes);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (user) {
      loadReflections(false); // Recarregar do início quando sortBy mudar
    }
  }, [sortBy]);

  // Função para carregar apenas a contagem de comentários (mais rápido)
  const loadCommentCount = async (reflectionId: string) => {
    try {
      // Buscar todos os comentários e filtrar localmente (mais simples que usar where com !=)
      const commentsQuery = query(
        collection(db, "reflections", reflectionId, "comments")
      );
      const snapshot = await getDocs(commentsQuery);
      // Filtrar comentários deletados localmente
      const count = snapshot.docs.filter(doc => !doc.data().deleted).length;
      setCommentCounts(prev => new Map(prev).set(reflectionId, count));
    } catch (error) {
  ...
  const loadComments = async (reflectionId: string) => {
    try {
      // Buscar todos os comentários e filtrar/ordenar localmente (mais simples e confiável)
      // Não usar orderBy no Firestore para evitar necessidade de índice
      const commentsQuery = collection(db, "reflections", reflectionId, "comments");
      const snapshot = await getDocs(commentsQuery);
      ...
      // Ordenar localmente por data de criação (mais antigo primeiro)
      ...
      setCommentCounts(prev => new Map(prev).set(reflectionId, commentsList.length));
    } catch (error) {
      console.error("Error loading comments:", error);
      // Em caso de erro, limpar comentários para esta reflexão
      setComments(prev => {
  ...
  const handleComment = async (reflectionId: string) => {
    ...
    try {
      ...
      // Atualizar contagem imediatamente (otimista)
      setCommentCounts(prev => {
        const newMap = new Map(prev);
        const currentCount = newMap.get(reflectionId) || 0;
        newMap.set(reflectionId, currentCount + 1);
        return newMap;
      });
      // Carregar comentários completos se estiverem abertos
      if (showComments.has(reflectionId)) {
        loadComments(reflectionId);
      } else {
        // Apenas atualizar a contagem
        loadCommentCount(reflectionId);
      }
    } catch (error) {
  ...
  const handleDeleteComment = async (reflectionId: string, commentId: string) => {
    ...
    try {
      ...
      setCommentCounts(prev => {
        const newMap = new Map(prev);
        const currentCount = newMap.get(reflectionId) || 0;
        newMap.set(reflectionId, Math.max(0, currentCount - 1));
        return newMap;
      });
      // Carregar comentários completos se estiverem abertos
      if (showComments.has(reflectionId)) {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }}>
      ...
      <main className="max-w-4xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-8">
        ...
        {filteredReflections.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredReflections.map((reflection, index) => {
              ...
              const commentCount = commentCounts.get(reflection.id) ?? reflectionComments.length;
                    ...
                    <div className="mt-4 animate-fade-in">
                      ...
                      <div className="flex items-center gap-3 sm:gap-4 pt-4" style={{ borderTop: '1px solid var(--border-subtle)' }}>
                        ...
                        >
                          ...
                          <span className="font-medium text-sm sm:text-base">{commentCount}</span>
                          {showCommentsForThis ? (
        ...
        {/* Botão Carregar Mais */}
        {hasMore && reflections.length > 0 && !loading && (
